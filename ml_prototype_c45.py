...
note by ferris: c45的特点：
1. 在每个几点，划分字段的每个唯一值对应一个独立的分支
2. 前面使用过的划分字段，不会在后面重复使用
...
def build_tree(D, A, threshold = 0.0001, T = -1, Tree = None, algo = "ID3"):  
    ...
    if (None == Tree):  # lazily初始化树对象
        Tree = Node(None, D)  
    subset = devide_set(D, T) 

    if (len(subset) <= 1):  # 异常处理：数据集只有一种结果
        for key in subset.keys():  
            Tree.result = key  
        del(subset)  # 删除中间数据
        return Tree  

    if (len(A) <= 0):  # 异常处理：没有特征集
        Tree.result = get_result(D)  
        return Tree  

    use_gain_ratio = False if algo == "ID3" else True  # 选择模型

    max_gain = 0.0  
    for a in A:  
        gain = info_gain(D, a, return_ratio = use_gain_ratio)  
        if (gain > max_gain):  # 更新追踪器，保存最大信息增益和对应的特征
            max_gain = gain  
            attr_id = a 

    if (max_gain < threshold):  # 信息增益低于阈值，停止成长
        Tree.result = get_result(D)  
        return Tree 

    Tree.attr = attr_id  # 继续成长
    subD = devide_set(D, attr_id)  # 子节点赋值
    del(D[:]) # 删除中间数据。数据已经存到子节点
    Tree.dataset = None  # 只有终端节点才有数据
    A.discard(attr_id) # 从特征集中排查已经使用过的特征 

    for key in subD.keys():  
        tree = Node(Tree, subD.get(key))  
        Tree.childs[key] = tree  
        build_tree(subD.get(key), A, threshold, T, tree)  # 递归
    return Tree  
